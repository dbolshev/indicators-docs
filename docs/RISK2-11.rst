...
Формула розрахунку
==================
1. Якщо в процедурі немає зміни до контрактів ``data.changes`` таких, що мають ``data.changes.status = 'active'`` та ``data.changes.rationaleTypes = 'itemPriceVariation'``, індикатор приймає значення ``-2``, розрахунок завершується
...
3. Знаходимо зміни до контрактів ``data.changes`` такі, що мають ``data.changes.status = 'active'`` та ``data.changes.rationaleTypes = 'itemPriceVariation'``
4. Якщо об'єктів з пункту 2 знайдено 3 чи більше, індикатор приймає значення ``1``.т У іншому випадку індикатор приймає значення ``0``.
